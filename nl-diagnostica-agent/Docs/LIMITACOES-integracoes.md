...
**Por quê:** a API é só de *leitura + decisão* (pull, favoritar, descartar). Qualquer cadastro manual teria que ser feito dentro do painel web da Effecti.
...
**Consequência prática:** o pull diário traz *tudo* que o perfil da conta captura nas últimas 24h. Se vier muito lixo (ou faltar coisa), o ajuste é no painel da Effecti — não no nosso código.
...
**Não disponível.** A integração é *pull* (nós perguntamos); a Effecti não nos notifica quando chega edital novo. Por isso existe o cron + botão manual.
...
Quando a fonte não manda itens (acontece em ambas), o match cai no *fallback* pelo `objeto` — menos preciso. A Super Triagem é o complemento indicado nesses casos.
...
|---|---|---|
| Reprocesso em lotes de 40 | `nl_rematch_all(p_limit=40)` em loop | *Statement timeout* do Supabase em transações longas (~350 editais de uma vez estourava) |
