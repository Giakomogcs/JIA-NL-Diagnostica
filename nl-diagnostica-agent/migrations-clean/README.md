...
| 9     | `009_kanban_gestao.sql`      | Gestão dos editais aceitos (board/kanban): colunas `kanban_stage`/gestão em `nl_edital`, `nl_board_editais()`, `nl_update_gestao()`                                                                                                                        |
| 10    | `010_editais_acoes_lote.sql` | `nl_dashboard_editais()` +`p_sort`/`p_ocultar_vencidos`; `nl_reject_sugeridos()` (recusa em lote); `nl_delete_recusados()` (apagar recusados, admin)                                                                                                       |
